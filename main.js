...
async function getData(segment) {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      const data = await response.json(); // Перевірка статусу відповіді
      console.log('Завдання #1. ....................................');
      console.log('Відповідь отримано:', data); // Обробка відповіді від сервера
    } 
    else {
      console.error('Запит не вдався, статус:', response.status);// Якщо статус відповіді не успішний, виводимо помилку
    }
  }
   catch (error) {
    console.log(`Помилка`, error.message); // Якщо виникає помилка, виводимо її
  }
}
...
async function postData(segment, data) {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      console.log('Відповідь отримано:', result);
    }
    else {
      console.error('Запит не вдався, статус:', response.status);
    }
  }
  catch (error) {
...
putData(1, { title: 'foo', body: 'bar', userId: 1 })
...
async function patchData(id, data) {try {
    const response = await fetch(`https://jsonplaceholder.typicode.com/posts/${id}`, { 
    method: 'PATCH',
    headers: { 'Content-Type': 'application/json' ,},
    body: JSON.stringify(data),
...
patchData(1, { title: 'foo', body: 'bar', userId: 1 })
...
async function deleteData(id) {
  try {
    ...
    if (response.status >= 200 && response.status < 300) {
      ...
      const result = await response.json();
      console.log(`Post with id ${id} has been successfully deleted.`, result);
    }
    else {
